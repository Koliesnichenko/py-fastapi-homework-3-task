...
async def user_register(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"A user with the same email {user.email} already exists."
        )
    ...
    if not user_group:
        raise HTTPException(500, detail="User group not found")
    try:
...
@router.post("/activate/", status_code=status.HTTP_200_OK, response_model=UserRegistrationResponseSchema)
